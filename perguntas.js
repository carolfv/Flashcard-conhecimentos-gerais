criaCartao(
    'Astronomia',
    'Qual é o maior planeta do sistema solar?',
    'O maior planeta do sistema solar é Júpiter.'
)
criaCartao(
    'Geografia',
    'Qual é a capital da Austrália?',
    'A capital da Austrália é Canberra.'
)
criaCartao(
    'Literatura',
    'Quem escreveu "Dom Quixote"?',
    '"Dom Quixote" foi escrito por Miguel de Cervantes.'
)
criaCartao(
    'Gastronomia',
    'Qual é o principal ingrediente do guacamole?',
    'O principal ingrediente do guacamole é o abacate.'
)
criaCartao(
    'Biologia',
    'Qual animal é conhecido como o "rei da selva"?',
    'O leão é conhecido como o "rei da selva".'
)
criaCartao(
    'Química',
    'Qual é a fórmula química da água?',
    'A fórmula química da água é H₂O.'
)
criaCartao(
    'Geografia',
    'Qual é o continente mais populoso do mundo?',
    'O continente mais populoso do mundo é a Ásia.'
)
criaCartao(
    'Economia',
    'Qual é o nome da moeda utilizada no Japão?',
    'A moeda utilizada no Japão é o iene'
)
criaCartao(
    'arte',
    'Quem pintou a Mona Lisa?',
    'A Mona Lisa foi pintada por Leonardo da Vinci.'
)
criaCartao(
    'Ciências',
    'Qual é o principal gás responsável pelo efeito estufa?',
    'O principal gás responsável pelo efeito estufa é o dióxido de carbono (CO₂).'
)
criaCartao(
    'Geografia',
    'Qual é a capital da França?',
    'A capital da França é Paris.'
)
criaCartao(
    'Programação',
    'Qual é a linguagem de programação mais popular atualmente?',
    'ma das linguagens de programação mais populares atualmente é o Python.'
)